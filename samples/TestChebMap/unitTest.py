...
class TestChebMap(unittest.TestCase):

    def __init__(self, testName, dir, execCmd='mpirun -n 8 ../tmp/pizza.exe', 
                 precision=1e-8):
        super(TestChebMap, self).__init__(testName)
        self.dir = dir
        self.precision = precision
        self.execCmd = execCmd
        self.startDir = os.getcwd()
        self.description = "Test remapping on a K-T-L grid"
    ...
    def setUp(self):
        ...
        os.chdir(self.dir)
        cmd = f'{self.execCmd} {self.dir}/inputStart.nml'
        sp.call(cmd, shell=True, stdout=open(os.devnull, 'wb'),
                stderr=open(os.devnull, 'wb'))
        cmd = f'{self.execCmd} {self.dir}/{inpFile}'
        sp.call(cmd, shell=True, stdout=open(os.devnull, 'wb'),
                stderr=open(os.devnull, 'wb'))
