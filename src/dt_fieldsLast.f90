module fieldsLast
   ...
   use namelists, only: l_heat, l_chem, l_finite_diff, l_phase_field
   use time_array
   ...
   subroutine initialize_fieldsLast(nMstart,nMstop,n_m_max,nRstart,nRstop,n_r_max,&
      ...
      if ( l_finite_diff ) then
         ...
         if ( l_phase_field ) then
            call dphidt%initialize(1, n_m_max, nRstart, nRstop, nold, &
                &                  nexp, nimp, l_allocate_exp=.true.)
         else
            call dphidt%initialize(1, 1, 1, 1, nold, nexp, nimp, l_allocate_exp=.true.)
         end if
      else
         call dpsidt%initialize(nMstart, nMstop, 1, n_r_max, nold, nexp, nimp)
         if ( l_heat ) call dTdt%initialize(nMstart, nMstop, 1, n_r_max, nold, &
                            &               nexp, nimp)
         if ( l_chem ) call dxidt%initialize(nMstart, nMstop, 1, n_r_max, nold, &
                            &                 nexp, nimp)
         if ( l_phase_field ) call dphidt%initialize(nMstart, nMstop, 1, n_r_max, &
                                   &                 nold, nexp, nimp)
         ...
         if ( l_phase_field ) n_mloc_fields = n_mloc_fields+1
         ...
         if ( l_heat ) then
            ...
            if ( l_chem ) then
               ...
               &                                                      1:n_r_max,6,1:nexp)
               if ( l_phase_field ) then
                  dphidt%expl(nMstart:,1:,1:) =>                          &
                  &              dt_fields_Mloc_container(nMstart:nMstop, &
                  &                                       1:n_r_max,7,1:nexp)
               end if
            else
               if ( l_phase_field ) then
      ...
      if ( l_finite_diff ) then
         ...
         if ( l_heat ) then
            ...
            if ( l_chem ) then
               ...
               if ( l_phase_field ) then
                  dphidt_Rloc(1:,nRstart:) => dt_fields_Rloc_container(1:n_m_max,nRstart:nRstop,7)
               end if
            else
               if ( l_phase_field ) then
                  dphidt_Rloc(1:,nRstart:) => dt_fields_Rloc_container(1:n_m_max,nRstart:nRstop,5)
               end if
            end if
         else
            if ( l_chem ) then
               ...
               if ( l_phase_field ) then
                  dphidt_Rloc(1:,nRstart:) => dt_fields_Rloc_container(1:n_m_max,nRstart:nRstop,3)
               end if
            end if
         end if
