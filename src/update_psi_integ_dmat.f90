         ...
         if ( l_ek_pump ) then
            ...
            do n_m=nMstart,nMstop
               ...
               if ( m == 0 ) then
                  ...
                  call LHS_om_mat(n_m)%initialize(10, 10, 4, n_cheb_max)
            ...
            do n_m=nMstart,nMstop
               ...
               if ( m == 0 ) then
                  ...
                  call LHS_om_mat(n_m)%initialize(8, 8, 4, n_cheb_max)
...
!------------------------------------------------------------------------------
   subroutine update_psi_int(psi_Mloc, om_Mloc, us_Mloc, up_Mloc, dVsOm_Mloc, &
      ...
      do n_r=1,n_r_max
         do n_m=nMstart, nMstop
            ...
            if ( m /= 0 ) then
               ...
               !-- Coriolis force has to be treated explicitly in this case:
               ! domdt%expl(n_m,n_r,1)=domdt%expl(n_m,n_r,1)-   &
      ...
      if ( l_rank_has_m0 .and. l_vphi_bal_calc ) then
         do n_r=1,n_r_max
            vp_bal%rey_stress(n_r)=real(domdt%expl(m2idx(0),n_r,1))
         end do
      end if
      ...
      runStart = MPI_Wtime()
      call rscheme%costf1(domdt%expl(:,:,1), nMstart, nMstop, n_r_max)
      runStop = MPI_Wtime()
      ...
      do n_m=nMstart,nMstop
         ...
         do n_cheb=1,n_cheb_max
            rhs(n_cheb)=domdt%expl(n_m,n_cheb,1)
         end do
         ...
         do n_cheb=1,n_cheb_max
            domdt%expl(n_m,n_cheb,1)=rhs(n_cheb)
         end do

         !-- Deliasing
         do n_cheb=n_cheb_max+1,n_r_max
            domdt%expl(n_m,n_cheb,1)=zero
         end do
      end do
      ...
      do n_m=nMstart,nMstop
         ...
         runStart = MPI_Wtime()
         call LHS_om_mat(n_m)%solve(rhs,n_cheb_max)
         runStop = MPI_Wtime()
         if ( runStop > runStart ) then
         ...
         if ( m == 0 ) then
            ...
            call LHS_psi_mat(n_m)%solve(rhs,n_cheb_max)
...
!------------------------------------------------------------------------------
   subroutine get_psi_rhs_imp_int(om_Mloc, up_Mloc, om_old, dom_imp_Mloc_last, &
      ...
      if ( l_calc_lin_rhs ) then
         ...
         do n_cheb=n_cheb_max+1,n_r_max
            do n_m=nMstart,nMstop
               dom_imp_Mloc_last(n_m,n_cheb)=zero
            end do
         end do
