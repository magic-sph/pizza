program pizza
   ...
   use namelists, only: read_namelists, write_namelists, tag
   ...
   !-- Start fields
   call get_start_fields(time, dt, dtNew)
   ...
   if ( rank == 0 ) then
      do n=1,2
         ...
         write(n_out,'(''   start dt   ='',1p,ES16.4)') dt
         write(n_out,'(''   start dtNew='',1p,ES16.4)') dtNew
      end do
   end if

   !-- Time integration
   call time_loop(time, dt, dtNew)
