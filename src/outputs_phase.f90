module outputs_phase
   ...
   use radial_functions, only: r, rscheme, tcond
   use namelists, only: r_cmb, tag, ra, pr, raxi, sc, radratio, stef, &
       &                tmelt, ek
   use mem_alloc, only: bytes_allocated
   use truncation, only: n_r_max, n_phi_max
...
contains
   ...
   subroutine initialize_out_phase()
      ...
      bytes_allocated=bytes_allocated+2*n_phi_max*nR_per_rank*SIZEOF_DEF_REAL
      ...
      bytes_allocated=bytes_allocated+2*n_r_max*(nPstop-nPstart+1)*SIZEOF_DEF_REAL

      allocate( rmelt_loc(nPstart:nPstop) )
      rmelt_loc(:)=0.0_cp
      bytes_allocated=bytes_allocated+(nPstop-nPstart+1)*SIZEOF_DEF_REAL
...
!---------------------------------------------------------------------------------------
   subroutine finalize_out_phase()
      ...
      deallocate( rmelt_loc )
      deallocate( volS, ekinS, ekinL )
...
!---------------------------------------------------------------------------------------
   subroutine write_out_phase(time)
      ...
      real(cp) :: rmelt_global(n_phi_max)
