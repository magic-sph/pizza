module update_psi_coll_dmat
   ...
   real(cp), allocatable :: psiMat(:,:,:), omMat(:,:,:), assMat(:,:,:)
   ...
   integer,  allocatable :: psiPivot(:,:), omPivot(:,:), assPivot(:,:)
   ...
   &         get_psi_rhs_imp_coll_dmat, finish_exp_psi_coll_dmat, assemble_psi_coll_dmat

contains

   subroutine initialize_om_coll_dmat(tscheme)
...
!------------------------------------------------------------------------------
   subroutine finalize_om_coll_dmat(tscheme)
   ...
   subroutine update_om_coll_dmat(psi_Mloc, om_Mloc, dom_Mloc, us_Mloc, up_Mloc, &
      ...
      do n_m=nMstart,nMstop
         ...
         if ( m == 0 ) then ! Axisymmetric component
            ...
            do n_r=2,n_r_max-1
               ...
               if ( l_buo_imp ) then
                  if ( l_heat ) then
                     rhs(n_r)=work_Mloc(n_m,n_r)-tscheme%wimp_lin(1)*BuoFac*rgrav(n_r)&
                     &        *or1(n_r)*ci*real(m,cp)*temp_Mloc(n_m,n_r)
                     if ( l_chem ) then
                        rhs(n_r)=rhs(n_r)-tscheme%wimp_lin(1)*ChemFac*rgrav(n_r)*&
                        &        or1(n_r)*ci*real(m,cp)*xi_Mloc(n_m,n_r)
                     end if
                  else
                     if ( l_chem ) then
            ...
            runStart = MPI_Wtime()
            call solve_full_mat(omMat(:,:,n_m), n_r_max, n_r_max, omPivot(:, n_m), rhs(:))
            runStop = MPI_Wtime()
      ...
      if ( tscheme%istage == tscheme%nstages ) then
         call get_psi_rhs_imp_coll_dmat(up_Mloc, om_Mloc, dom_Mloc, temp_Mloc, &
              &                         xi_Mloc, dpsidt, 1, vp_bal,            &
              &                         vort_bal, tscheme%l_imp_calc_rhs(1),   &
              &                         l_in_cheb_space=.true.)
      else
         call get_psi_rhs_imp_coll_dmat(up_Mloc, om_Mloc, dom_Mloc, temp_Mloc,   &
              &                         xi_Mloc, dpsidt, tscheme%istage+1,       &
              &                         vp_bal, vort_bal,                        &
...
!------------------------------------------------------------------------------
   subroutine finish_exp_psi_coll_dmat(us_Mloc, up_Mloc, om_Mloc, dVsOm_Mloc, &
              &                        temp_Mloc, xi_Mloc, dpsi_exp_last, vort_bal)
...
!------------------------------------------------------------------------------
   subroutine get_psi_rhs_imp_coll_dmat(up_Mloc, om_Mloc, dom_Mloc, temp_Mloc, &
              &                         xi_Mloc, dpsidt, istage, vp_bal,       &
              &                         vort_bal, l_calc_lin, l_in_cheb_space)
...
!------------------------------------------------------------------------------
   subroutine assemble_psi_coll_dmat(psi_Mloc, us_Mloc, up_Mloc, om_Mloc, temp_Mloc, &
      ...
      call get_psi_rhs_imp_coll_dmat(up_Mloc, om_Mloc, dom_Mloc, temp_Mloc,  &
           &                         xi_Mloc, dpsidt, 1, vp_bal, vort_bal,   &
           &                         tscheme%l_imp_calc_rhs(1),              &
           &                         l_in_cheb_space=.false.)

   end subroutine assemble_psi_coll_dmat
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
   subroutine get_assMat(m, assMat, assPivot)
      ...
      !-- Output variables
      real(cp), intent(out) :: assMat(n_r_max,n_r_max)
